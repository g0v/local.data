...
exports.parseSpreadsheetUrl = function(url) {
  var regex = /.*spreadsheets?\/(?:ccc?.*key=|d\/)([^#?&+]+)[^#]*(#gid=([\d]+).*)?/,
    out = { key: '', sheet: 0 }
      ;
  if (!url) {
    return out;
  ...
  Tabletop.init( {
    ...
    callback: function(data, tabletop) {
      ...
      if (!config.get('places')) {
        var places = config.get('configUrl').replace(/gid=(\d+)/, function(_, _1) {
          return 'gid=' + (+_1 + 1)
        })
...
var _loadQuestions = function (db, callback) {
  exports.getCsvData(config.get('questions'), function(err, data) {
    ...
    // run the questions through transformData
    db.questions = transformData(data);

    function transformData(data) {
        // calls any transformations on the backend question data
        ...
        data.forEach(function(question) {
            // call transform functions on each question
            transformDepends(lookup, question);
        });

        return data;
    }

    function transformDepends(lookup, question) {
        // transforms depends arguments into resolved question objects
        var rawDependants,
        resolvedDependants = [];

        if (question.dependants) {
            rawDependants = question.dependants.split(',')
        }

        if (rawDependants) {
            rawDependants.forEach(function (dependant) {
                resolvedDependants.push(lookup[dependant]);
            });
            question.dependants = resolvedDependants;
        } else {
            question.dependants = [];
        }
...
_loadDatasets = function (db, callback) {
  request.get({
    url: 'http://data.g0v.tw/api/dataset?f%5B0%5D=field_census%3A1',
    json: true
  }, function(err, data){
    var body, res$, i$, ref$, len$, dataset;
    body = data.body;
    res$ = [];
    for (i$ = 0, len$ = (ref$ = body.data).length; i$ < len$; ++i$) {
      dataset = ref$[i$].dataset;
      res$.push((dataset.id = "g0v:" + dataset.uuid, dataset));
    }
    data = res$.sort(function(arg$, arg1$){
      var a, b;
      a = arg$.title;
      b = arg1$.title;
...
exports.translateQuestions = function(questions, lang) {
  // Like translateRows but take depedants field into consideration
  return _.map(questions, function(question) {
    var ret = exports.translate(question, lang);
    if (ret.dependants)
      ret.dependants = exports.translateRows(ret.dependants, lang);
    return ret;
  });
}
